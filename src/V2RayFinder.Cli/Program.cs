...
var args = Args.Parse(Environment.GetCommandLineArgs()[1..]);
...
var pipeline = new Pipeline(
    checkHealth:       args.CheckHealth,
    fetchConcurrency:  args.Concurrency,
    limit:             args.Limit,
    fetchTimeout:      TimeSpan.FromSeconds(args.Timeout)
);
...
if (result.Scores.Count > 0)
{
    ...
    int n = Math.Min(args.TopN, result.Scores.Count);
    for (int i = 0; i < n; i++)
    {
...
if (args.OutputFile != null)
{
    ...
    await File.WriteAllLinesAsync(args.OutputFile, lines, cts.Token);
    Console.WriteLine($"[✓] Saved {lines.Count} configs to {args.OutputFile}");
}
